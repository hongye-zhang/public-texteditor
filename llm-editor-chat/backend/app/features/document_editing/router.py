...
Your response will be automatically processed to replace the selected text in the editor, so focus on producing high-quality content that fulfills the user's modification request.
"""
                ...
                if event.get("type") == "action" and event.get("action"):
                    ...
                    if action_match:
                        ...
                        try:
                            ...
                            # 使用嵌套action中的content
                            if nested_action.get("type") == "replace-text" and "payload" in nested_action and "content" in nested_action["payload"]:
                    ...
                    # 确保动作类型为replace
                    if action.get("type") != "replace-text":
                        action["type"] = "replace-text"
                    
                    # 无论动作类型是什么，都使用前端传递的位置信息
                    if "payload" in action:
                        # 使用前端期望的from和to属性名称，而不是start和end
                        action["payload"]["from"] = request.start_position
                        action["payload"]["to"] = request.end_position
